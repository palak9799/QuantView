...
app.post("/api/quantview/interview/evaluate-question", async (req, res) => {
  ...
  if (interviewType === "aptitude") {
    ...
    const answerQualityScore = isCorrectAns ? 15 : Math.round(2 + Math.random() * 3);
    const questionAlignmentScore = isCorrectAns ? 15 : Math.round(1 + Math.random() * 3);

    const communicationScore = fillers === 0 
      ? Math.round(12 + Math.random() * 3) 
      : (fillers <= 2 ? Math.round(10 + Math.random() * 3) : Math.round(6 + Math.random() * 4));

    const confidenceScore = (pace >= 100 && pace <= 160) 
      ? Math.round(11 + Math.random() * 4) 
      : Math.round(8 + Math.random() * 3);

    const fluencyScore = fillers === 0 
      ? Math.round(13 + Math.random() * 2) 
      : (fillers <= 2 ? Math.round(11 + Math.random() * 3) : Math.round(7 + Math.random() * 4));

    const pronunciationScore = Math.max(6, Math.min(15, Math.round(speechConfidence * 14 + Math.random() * 2 - 1)));
    const eyeContactScore = isGazeStable ? Math.round(12 + Math.random() * 3) : Math.round(7 + Math.random() * 4);
    const bodyLanguageScore = isPostureAligned ? Math.round(12 + Math.random() * 3) : Math.round(7 + Math.random() * 4);

    const overallScore = Math.round((answerQualityScore + communicationScore + confidenceScore + fluencyScore + pronunciationScore + eyeContactScore + bodyLanguageScore) / 7);

    const goodPoints = isCorrectAns 
      ? ["Perfect! You solved the aptitude problem correctly."] 
      : ["Analytical logic was attempted for the logical reasoning problem."];

    const improvements = isCorrectAns 
      ? ["None. Keep up the great mathematical and logical reasoning!"] 
      : ["Review the correct answer and practice similar aptitude problems to improve your speed and accuracy."];

    const scoreExplanations = {
      answerQuality: isCorrectAns ? `Correct Option Selected (${answerQualityScore}).` : `Incorrect Option Selected (${answerQualityScore}).`,
      communication: `Spoken pacing and response delivery was clear and steady (${communicationScore}).`,
      confidence: `Answered logical logic with a focused presence (${confidenceScore}).`,
      fluency: `Conversation rhythm scored based on filler transitions (${fluencyScore}).`,
      pronunciation: `Speech articulation metrics were recorded cleanly (${pronunciationScore}).`,
      eyeContact: `Eye tracking registered connection metrics at (${eyeContactScore}).`,
      bodyLanguage: `Centered postural alignments registered at (${bodyLanguageScore}).`,
      overall: isCorrectAns ? "Perfect solve!" : "Incorrect solve."
    };
  ...
  if (!ai) {
    ...
    let answerQualityScore = 8;
    let communicationScore = 8;
    let confidenceScore = 8;
    let fluencyScore = 8;
    let pronunciationScore = Math.round(speechConfidence * 15);
    let eyeContactScore = isGazeStable ? 14 : 9;
    let bodyLanguageScore = isPostureAligned ? 14 : 9;
    
    let goodPoints: string[] = [];
    let improvements: string[] = [];

    // STRICT GRADING RUBRIC IMPLEMENTATION (MAXIMUM 15 POINTS SCALE)
    if (isSilent) {
      ...
      if (wordCount <= 2) {
        answerQualityScore = Math.round(1 + Math.random() * 2); // 1 - 3
      } else if (wordCount < 10) {
        answerQualityScore = Math.round(4 + Math.random() * 2); // 4 - 6
      } else if (wordCount < 20) {
        answerQualityScore = Math.round(6 + Math.random() * 3); // 6 - 9
      } else if (wordCount < 30) {
        answerQualityScore = Math.round(8 + Math.random() * 3); // 8 - 11
      } else if (wordCount < 50) {
        answerQualityScore = Math.round(10 + Math.random() * 3); // 10 - 13
      } else if (wordCount < 100) {
        answerQualityScore = Math.round(12 + Math.random() * 2); // 12 - 14
      } else {
        ...
        answerQualityScore = hasMetrics ? Math.round(14 + Math.random() * 1) : Math.round(11 + Math.random() * 2); // 11-15
      }

      // 2. Communication: based on clarity, length, sentence structure
      if (wordCount <= 2) {
        communicationScore = Math.round(1 + Math.random() * 1); // 1 - 2
      } else if (wordCount < 10) {
        communicationScore = Math.round(4 + Math.random() * 3); // 4 - 7
      } else {
        let commBase = 11;
        if (fillers > 4) commBase -= 2;
        if (fillers === 0) commBase += 2;
        communicationScore = Math.max(4, Math.min(15, Math.round(commBase + Math.random() * 3 - 1.5)));
      }

      // 3. Confidence Score: based on pacing, stability and duration
      if (wordCount <= 2) {
        confidenceScore = Math.round(1 + Math.random() * 2); // 1 - 3
      } else {
        let confBase = 11;
        if (pace < 90 || pace > 170) confBase -= 2;
        if (isGazeStable) confBase += 1;
        confidenceScore = Math.max(4, Math.min(15, Math.round(confBase + Math.random() * 4 - 2)));
      }

      // 4. Fluency Score: highly sensitive to fillerWordsCount and speaking pace
      if (wordCount <= 2) {
        fluencyScore = Math.round(1 + Math.random() * 1); // 1 - 2
      } else {
        let fluBase = 13;
        if (fillers === 0) fluBase = 14;
        else if (fillers <= 2) fluBase = 12;
        else if (fillers <= 5) fluBase = 9;
        else fluBase = 6;

        if (pace < 100 || pace > 180) fluBase -= 1;
        fluencyScore = Math.max(3, Math.min(15, Math.round(fluBase + Math.random() * 2 - 1)));
      }

      // 5. Pronunciation Score: based on speechConfidence
      if (wordCount <= 2) {
        pronunciationScore = Math.round(2 + Math.random() * 2); // 2 - 4
      } else {
        let pronBase = Math.round(speechConfidence * 14);
        pronunciationScore = Math.max(4, Math.min(15, Math.round(pronBase + Math.random() * 2 - 1)));
      }

      // 6. Eye Contact Score: based on gaze stability
      if (isGazeStable) {
        eyeContactScore = Math.round(13 + Math.random() * 2); // 13 - 15
      } else {
        eyeContactScore = Math.round(6 + Math.random() * 4); // 6 - 10
      }

      // 7. Body Language Score: based on posture alignment
      if (isPostureAligned) {
        bodyLanguageScore = Math.round(13 + Math.random() * 2); // 13 - 15
      } else {
        bodyLanguageScore = Math.round(6 + Math.random() * 4); // 6 - 10
      }

      // Build fallback improvements and good points based on the actual scores
      if (wordCount < 10) {
        goodPoints.push("Detected initial vocal activity.");
        improvements.push("Your answer length is extremely short (under 10 words). You must expand with structured STAR responses.");
      } else if (wordCount < 30) {
        goodPoints.push("Vocal tone is present, but response content structure is incomplete.");
        improvements.push("An incomplete or partial answer cannot receive high alignment score. Elaborate on your technical results.");
      } else {
        goodPoints.push("Structured communication with clear voice articulation.");
        if (answerQualityScore < 12) {
    ...
    if (questionAlignmentScore < 8) {
      overallScore = Math.min(overallScore, 6);
    } else if (questionAlignmentScore < 14) {
    ...
    const scoreExplanations = {
      ...
      confidence: confidenceScore === 0
        ? `Confidence is 0/15: no sound recorded.`
        : `Vocal assertion rate scored at ${confidenceScore}/15 based on verbal speed.`,
      
      fluency: fluencyScore === 0
        ? `Fluency index is 0/15. No continuous speech flow detected.`
        : `Fluent pacing recorded at ${fluencyScore}/15 over the session segment.`,
      
      pronunciation: pronunciationScore === 0
        ? `Pronunciation score is 0/15. No speech articulation observed.`
        : `Articulative phoneme enunciation graded at ${pronunciationScore}/15.`,
      
      eyeContact: eyeContactScore === 0
        ? `Eye contact is 0/15 because no response activity was observed.`
        : `Steady gaze tracking registered connection metrics at ${eyeContactScore}/15.`,
      
      bodyLanguage: bodyLanguageScore === 0
        ? `Posture score is 0/15 because no response activity was observed.`
        : `Centered posture alignment metric scored at ${bodyLanguageScore}/15.`,
      
      overall: `This single feedback round evaluated at ${overallScore}/15. Focus on key suggestions to sustain this professional presence.`
    };
  ...
  try {
    const evaluatePrompt = `
      ...
      - Posture is centered/aligned: ${telemetry?.postureAligned !== false ? "Yes" : "No"}
      
      STRICT EVALUATION & GRADING INSTRUCTIONS (15-POINT SCALE SYSTEM):
      You are a realistic, professional recruiter. You MUST evaluate the response strictly, honestly, and realistically out of 15. The absolute maximum score is 15 points.
      No metric should ever exceed 15.
      ...
      - Average performance should remain below 10.
      - A score of 15 should be awarded only for truly excellent performance.
      ...
      - If Question Alignment Score (0-15) is extremely low (0-3), the overallScore (0-15) MUST NOT exceed 4 out of 15 under any circumstances, even if other parameters like posture, fluency, and eye contact are perfect.
      - If Question Alignment Score (0-15) is moderate (4-10), the overallScore (0-15) MUST NOT exceed 10 out of 15.
      - High overallScores (11+) should only be awarded when the Question Alignment Score is Excellent (11+) and the intent of the question is fully covered.
      
      CRITICAL - GRADING TIER RULES (YOU MUST ADHERE STRICTLY TO THESE SCORES out of 15, based on the spoken text content):
      ...
      2. ONLY 1-2 WORDS:
         ...
         - Score Bounds: 1 to 3 for overall and individual scores. Correct visual indicators if eyes and posture were steady, but cap everything strictly at 1-3.
         
      3. VERY SHORT ANSWER:
         - Criteria: Candidate spoke 3 to 9 words.
         - Score Bounds: 2 to 5 for overall and individual scores. Explanations must explain that this is too short to demonstrate competency.
         
      4. WEAK ANSWER:
         - Criteria: Candidate spoke 10 to 19 words.
         - Score Bounds: 4 to 7 for overall and individual scores.
         
      5. PARTIAL ANSWER / INCOMPLETE:
         - Criteria: Candidate spoke 20 to 29 words. The answer is incomplete, brief, or missing major technical context.
         - Score Bounds: 6 to 9 for overall and individual scores. If the answer is incomplete, score should not exceed 9.
         
      6. AVERAGE ANSWER:
         - Criteria: Candidate spoke 30 to 49 words. Basic details, but lacks quantitative results or STAR structuring.
         - Score Bounds: 8 to 10 for overall and individual scores. Average performance should remain below 10.
         
      7. GOOD ANSWER (COMMUNICATION QUALITY VERIFIED):
         - Criteria: Candidate spoke 50 to 99 words. Solid communication, clear explanation, relevant technical terminology, but lacks deep outcome metrics.
         - Score Bounds: 11 to 13 for overall and individual scores.
         
      8. EXCELLENT ANSWER (SUPREME LEVEL):
         - Criteria: Candidate spoke 100+ words. Perfect STAR format (Situation, Task, Action, Result), minimal filler words, stable gaze and posture, clear relevant frameworks, and explicit quantitative outcome metrics (percentages or numbers).
         - Score Bounds: 14 to 15 for overall and individual scores. Genuinely outstanding answer. Only award 15 for truly excellent performance.
         
      STRICT GRADING BOUNDARIES:
      - If only face movement, eye tracking, or basic voice presence is detected without real content analysis (or speech-to-text fails/no transcript starts), maximum score is 4.
      - Never award more than 10 unless the answer is complete, relevant, confident, and well-structured.
      - Maintain a highly realistic, conservative grading behavior. Do NOT give high scores automatically. Only genuinely strong performance should receive scores above 11.
      ...
      QUESTION ALIGNMENT ANALYSIS (0-15 SCALE):
      ...
      - Scoring Range (Maximum 15):
        - 0–3: Completely unrelated answer or meaningless response.
        - 4–7: Partially related answer with very limited information.
        - 8–11: Relevant answer but missing important details.
        - 12–15: Highly relevant answer that properly addresses the question.
      - High scores (12-15) should be awarded when the candidate directly answers the question and covers important expected points.
      - Low scores (0-3 or 4-7) should be awarded when the response is unrelated, vague, or does not address the question.
      ...
      Return the final output strictly as a JSON object matching this schema (with standard scores strictly from 0 to 15, relevance score strictly from 0 to 100, and alignment score strictly from 0 to 15):
      {
        "goodPoints": ["string", "string", ...],
        "improvements": ["string", "string", ...],
        "answerQualityScore": number (0-15),
        "communicationScore": number (0-15),
        "confidenceScore": number (0-15),
        "fluencyScore": number (0-15),
        "pronunciationScore": number (0-15),
        "eyeContactScore": number (0-15),
        "bodyLanguageScore": number (0-15),
        "overallScore": number (0-15),
        "questionRelevanceScore": number (0-100),
        "questionRelevanceFeedback": "string",
        "questionAlignmentScore": number (0-15),
        "questionAlignmentFeedback": "string",
        "questionAlignmentMissingPoints": ["string", "string", ...],
        "questionAlignmentSuggestions": "string",
        "scoreExplanations": {
          "answerQuality": "Explain why this score (out of 15) was awarded.",
          "communication": "Reasoning for communication, pacing, clarity, and stutters (out of 15).",
          "confidence": "Reasoning for vocal confidence, tone level, and assertiveness (out of 15).",
          "fluency": "Reasoning for speed and filler pauses (out of 15).",
          "pronunciation": "Reasoning on pronunciation enunciation (out of 15).",
          "eyeContact": "Reasoning on eye contact tracking (out of 15).",
          "bodyLanguage": "Reasoning on posture and facial alignments (out of 15).",
          "overall": "A brief summary of why overall rating (out of 15) was awarded."
        },
    ...
    result.fluencyScore = sanitizeScore(result.fluencyScore, 0); // Guarantee Fluency is never blank, default to 0/15
...
app.post("/api/quantview/interview/evaluate", async (req, res) => {
  ...
  if (!ai) {
    ...
    questions.forEach((q: any) => {
      ...
      totalComm += Math.min(15, q.communicationScore !== undefined ? q.communicationScore : 8);
      totalConf += Math.min(15, q.confidenceScore !== undefined ? q.confidenceScore : 8);
      totalEye += Math.min(15, q.eyeContactScore !== undefined ? q.eyeContactScore : 8);
      totalBody += Math.min(15, q.bodyLanguageScore !== undefined ? q.bodyLanguageScore : 8);
      totalQuality += Math.min(15, q.answerQualityScore !== undefined ? q.answerQualityScore : 8);
      totalOverall += Math.min(15, q.overallScore !== undefined ? q.overallScore : 8);
      totalRelevance += q.questionRelevanceScore !== undefined ? q.questionRelevanceScore : 75;
      totalAlignment += Math.min(15, q.questionAlignmentScore !== undefined ? q.questionAlignmentScore : 8);
    });

    const avgPace = samplesCount > 0 ? Math.round(totalPace / samplesCount) : 130;
    const commScore = samplesCount > 0 ? Math.round(totalComm / samplesCount) : 8;
    const confScore = samplesCount > 0 ? Math.round(totalConf / samplesCount) : 8;
    const eyeScore = samplesCount > 0 ? Math.round(totalEye / samplesCount) : 8;
    const bodyScore = samplesCount > 0 ? Math.round(totalBody / samplesCount) : 8;
    const qualityScore = samplesCount > 0 ? Math.round(totalQuality / samplesCount) : 8;
    const calculatedOverall = samplesCount > 0 ? Math.round(totalOverall / samplesCount) : 8;
    const avgRelevanceScore = samplesCount > 0 ? Math.round(totalRelevance / samplesCount) : 75;
    const avgAlignmentScore = samplesCount > 0 ? Math.round(totalAlignment / samplesCount) : 8;
    ...
    const faceScore = Math.min(15, Math.round(bodyScore * 1.02));
    const paceScore = avgPace >= 110 && avgPace <= 160 ? 13 : 9;
    ...
    const offlineEval = {
      ...
      mistakesMade: [
        qualityScore < 9 ? "Spoke very briefly on one or more questions, which reduced technical and behavioral evaluation depth." : "Missed expanding on STAR action criteria under pressure.",
      ...
      bodyLanguageFeedback: `Posture tracking stayed stable at ${bodyScore}/15. Make sure to sit comfortably centered and keep shoulders level.`,
      eyeContactFeedback: `Ocular scanner evaluated your camera concentration at ${eyeScore}/15. Focus on speaking straight toward the webcam rather than looking down to think.`,
  ...
  try {
    const evaluationPrompt = `
      ...
      STRICT METRICS AND GRADING RULES FOR CUMULATIVE SESSION (15-POINT SCALE SYSTEM):
      1. This overall evaluation must reflect honest, strict recruiter-level grading out of 15 maximum points. No score can exceed 15.
      2. Set overallScore and other scores (0 to 15) by strictly aggregating and averaging the scores of individual questions (questions have overallScore, answerQualityScore, communicationScore, etc. already populated).
      3. If the questions have low scores (e.g., if the user remained silent or gave very short answers which resulted in scores below 8), the cumulative overallScore, technicalPerformanceScore, communicationScore, voiceAnalysisScore etc. MUST be correspondingly low. Do not inject inflated default values (like 12+).
      4. Avoid automatic high or default grading. Realistic, objective assessments only. Only genuinely strong performance should receive scores above 11.
      5. SCORING SCALE:
         - 0–3 = Poor
         - 4–7 = Basic
         - 8–11 = Average
      ...
      - Highly aligned answers overall (directly answering questions, covering most expected topics) -> High score (12-15).
        Feedback: "Your answers directly addressed the questions and covered the expected key points."
      - Partially aligned answers overall -> Medium score (6-11).
        Feedback: "Your responses partially addressed the questions but missed some expected details."
      - Mostly irrelevant or vague answers overall -> Low score (0-5).
      ...
      Return the final output strictly as a JSON object matching this schema:
      {
        ...
        "communicationScore": number (0-15),
        "confidenceScore": number (0-15),
        "voiceAnalysisScore": number (0-15),
        "facialExpressionScore": number (0-15),
        "eyeContactScore": number (0-15),
        "bodyLanguageScore": number (0-15),
        "technicalPerformanceScore": number (0-15),
        "questionRelevanceScore": number (0-100),
        "questionRelevanceFeedback": "string",
        "questionAlignmentScore": number (0-15),
        "questionAlignmentFeedback": "string",
    ...
    res.json({
      ...
      communicationScore: 10,
      confidenceScore: 11,
      voiceAnalysisScore: 10,
      facialExpressionScore: 11,
      eyeContactScore: 10,
      bodyLanguageScore: 11,
      technicalPerformanceScore: 10,
      ...
      detailedAnalysisParagraph: "QuantView Coach evaluation: You communicated your concepts effectively, demonstrating strong foundation skills. Addressing micro stutters and stabilizing eye gaze will lock down an outstanding interview result (11/15).",
      practiceRecommendations: [
