...
export const getAlignmentStatus = (score: number, max: number = 100) => {
  const norm = max === 15 ? (score * 100 / 15) : (max === 50 ? score * 2 : (max === 20 ? score * 5 : score));
  ...
  const evaluateCurrentQuestionAndShowFeedback = async (question: string, answerText: string, updatedQs: MockQuestion[]) => {
    ...
    try {
      ...
      if (wordCount === 0) {
        ...
        if (wordCount <= 2) {
          qScore = Math.round(1 + Math.random() * 2); // 1 - 3
        } else if (wordCount < 10) {
          qScore = Math.round(4 + Math.random() * 2); // 4 - 6
        } else if (wordCount < 20) {
          qScore = Math.round(6 + Math.random() * 3); // 6 - 9
        } else if (wordCount < 30) {
          qScore = Math.round(8 + Math.random() * 3); // 8 - 11
        } else if (wordCount < 50) {
          qScore = Math.round(10 + Math.random() * 3); // 10 - 13
        } else if (wordCount < 100) {
          qScore = Math.round(12 + Math.random() * 2); // 12 - 14
        } else {
          ...
          qScore = hasMetrics ? Math.round(14 + Math.random() * 1) : Math.round(11 + Math.random() * 2); // 11-15
        }

        // 2. Communication Score
        if (wordCount <= 2) {
          cScore = Math.round(1 + Math.random() * 1); // 1 - 2
        } else if (wordCount < 10) {
          cScore = Math.round(4 + Math.random() * 3); // 4 - 7
        } else {
          let commBase = 11;
          if (fillerWordsCount > 4) commBase -= 2;
          if (fillerWordsCount === 0) commBase += 2;
          cScore = Math.max(4, Math.min(15, Math.round(commBase + Math.random() * 3 - 1.5)));
        }

        // 3. Confidence Score
        if (wordCount <= 2) {
          confScore = Math.round(1 + Math.random() * 2); // 1 - 3
        } else {
          let confBase = 11;
          if (speechPaceWpm < 90 || speechPaceWpm > 170) confBase -= 2;
          if (gazeStable) confBase += 1;
          confScore = Math.max(4, Math.min(15, Math.round(confBase + Math.random() * 4 - 2)));
        }

        // 4. Fluency Score
        if (wordCount <= 2) {
          fScore = Math.round(1 + Math.random() * 1); // 1 - 2
        } else {
          let fluBase = 13;
          if (fillerWordsCount === 0) fluBase = 14;
          else if (fillerWordsCount <= 2) fluBase = 12;
          else if (fillerWordsCount <= 5) fluBase = 9;
          else fluBase = 6;

          if (speechPaceWpm < 100 || speechPaceWpm > 180) fluBase -= 1;
          fScore = Math.max(3, Math.min(15, Math.round(fluBase + Math.random() * 2 - 1)));
        }

        // 5. Pronunciation Score
        if (wordCount <= 2) {
          pScore = Math.round(2 + Math.random() * 2); // 2 - 4
        } else {
          let pronBase = Math.round((pronunciationConfidence || 0.90) * 14);
          pScore = Math.max(4, Math.min(15, Math.round(pronBase + Math.random() * 2 - 1)));
        }

        // 6. Eye Contact Score
        if (gazeStable) {
          eyeScore = Math.round(13 + Math.random() * 2); // 13 - 15
        } else {
          eyeScore = Math.round(6 + Math.random() * 4); // 6 - 10
        }

        // 7. Body Language Score
        if (postureAligned) {
          bodyScore = Math.round(13 + Math.random() * 2); // 13 - 15
        } else {
          bodyScore = Math.round(6 + Math.random() * 4); // 6 - 10
        }

        // Build fallback improvements based on scores
        if (wordCount < 10) {
          fallbackImprovements.push("Your answer length is extremely short (under 10 words). You must expand with structured STAR responses.");
        } else if (wordCount < 30) {
          fallbackImprovements.push("An incomplete or partial answer cannot receive high alignment score. Elaborate on your technical results.");
        } else {
          if (qScore < 12) {
      ...
      if (wordCount === 0) {
        ...
        if (isTellMeAboutYourself) {
          ...
          if (aLower.includes("cricket") && aLower.includes("campus") && !aLower.includes("study") && !aLower.includes("project") && !aLower.includes("engineering") && !aLower.includes("b.tech") && wordCount < 15) {
            ...
            alignScore = Math.max(3, Math.min(15, Math.round(14 - timeDeductionScore / 2)));
            ...
            alignScore = Math.max(2, Math.min(10, Math.round(9 - timeDeductionScore / 2)));
            ...
            alignScore = Math.max(1, Math.min(5, Math.round(4 - timeDeductionScore / 2)));
          ...
          if (coveredCount === 3 && wordCount >= 40) {
            ...
            alignScore = Math.max(2, Math.min(10, Math.round(8 - timeDeductionScore / 2)));
            ...
            alignScore = Math.max(1, Math.min(4, Math.round(3 - timeDeductionScore / 2)));
      ...
      if (alignScore < 4) {
        finalizedOverall = Math.min(finalizedOverall, 4);
      } else if (alignScore < 11) {
        finalizedOverall = Math.min(finalizedOverall, 10);
      }

      const fallbackFeedback = {
        goodPoints: ["Sensory status indicators aligned and active"],
        ...
        scoreExplanations: {
          answerQuality: `Evaluated at ${qScore}. Answer content captured was: "${trimmed || "silence"}".`,
          communication: `Communication rating corresponds strictly to enunciation clarity and words delivered (${cScore}).`,
          confidence: `Vocal assertion rate scored at ${confScore} based on speech presence.`,
          fluency: `Fluency index represents conversational flow and transition pauses (${fScore}).`,
          pronunciation: `Speech recognition enunciation clarity graded at ${pScore}.`,
          eyeContact: `Steady gaze tracking registered connection metrics at ${eyeScore}.`,
          bodyLanguage: `Centered postural alignments registered at ${bodyScore}.`,
          overall: `Overall rating calculated at ${finalizedOverall} after penalizing for alignment level.`
        },
