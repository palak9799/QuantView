...
Overall Performance   : ${evaluation.overallScore} / 15
----------------------------------------------------------------------

=================== SENSORY CORE METRICS ===================
- Communication Skills Score   : ${evaluation.communicationScore} / 15
- Sustained Confidence Score   : ${evaluation.confidenceScore} / 15
- Voice Analysis Score         : ${evaluation.voiceAnalysisScore} / 15
- Facial Expression Score       : ${evaluation.facialExpressionScore} / 15
- Eye Contact Tracker Score    : ${evaluation.eyeContactScore} / 15
- Posture & Form Alignment     : ${evaluation.bodyLanguageScore} / 15
- Technical Depth Score        : ${evaluation.technicalPerformanceScore} / 15
              ...
              <div className="relative w-36 h-36 flex items-center justify-center">
                <svg className="absolute w-full h-full transform -rotate-90">
                  ...
                  <circle 
                    ...
                    strokeDasharray={`${2 * Math.PI * 62}`}
                    strokeDashoffset={`${2 * Math.PI * 62 * (1 - (overall !== undefined ? overall : 0) / 15)}`}
                    strokeLinecap="round"
                  />
                </svg>
                <div className="text-center z-10 space-y-0.5">
            ...
            {interviewType === "aptitude" ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
                  {[
                    { label: "Verbal Command", val: evaluation.communicationScore !== undefined ? evaluation.communicationScore : 11, max: 15, icon: FileText, color: "text-blue-400" },
                    { label: "Stability / Confidence", val: evaluation.confidenceScore !== undefined ? evaluation.confidenceScore : 11, max: 15, icon: Award, color: "text-teal-400" },
                    { titleSuffix: " (WPM)", label: "Voice Resonance", val: evaluation.voiceAnalysisScore !== undefined ? evaluation.voiceAnalysisScore : 11, max: 15, icon: Activity, color: "text-indigo-400 animate-pulse" },
                    { label: "Gaze Concentration", val: evaluation.eyeContactScore !== undefined ? evaluation.eyeContactScore : 11, max: 15, icon: Eye, color: "text-cyan-400" },
                    { label: "Question Relevance", val: evaluation.questionRelevanceScore !== undefined ? Math.round(evaluation.questionRelevanceScore * 15 / 100) : 11, max: 15, isRelevance: true, feedback: evaluation.questionRelevanceFeedback ?? "Your responses partially addressed the questions.", icon: Sparkles, color: "text-emerald-400" },
                    { label: "Question Alignment Score", val: evaluation.questionAlignmentScore !== undefined ? evaluation.questionAlignmentScore : 11, max: 15, isRelevance: true, feedback: evaluation.questionAlignmentFeedback ?? "Your responses aligned well with the question intents.", icon: Target, color: "text-pink-400 animate-pulse" }
                  ].map((m) => {
                    ...
                    <div className="md:col-span-4 bg-slate-950 p-5 rounded-xl border border-slate-800/80 flex flex-col justify-between">
                      <div>
                        ...
                        <div className="flex items-baseline space-x-2 mt-2">
                          <span className="text-3xl font-bold font-mono text-pink-400">{evaluation.questionAlignmentScore ?? 11}</span>
                          <span className="text-[10px] text-pink-500/80 font-mono">
                            {(evaluation.questionAlignmentScore ?? 11) >= 12 ? "HIGH ALIGNMENT" : (evaluation.questionAlignmentScore ?? 11) >= 8 ? "MODERATE ALIGNMENT" : "LOW ALIGNMENT"}
                          </span>
                        </div>
                  ...
                  {session.interviewType !== "aptitude" && q.questionRelevanceScore !== undefined && (
                    <div className="mt-3 pt-3 border-t border-slate-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-slate-900/20 p-3 rounded-lg border border-slate-900">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold font-mono text-emerald-400">{Math.round(q.questionRelevanceScore * 15 / 100)}</span>
                      </div>
